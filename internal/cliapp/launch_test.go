...
// Claude Code re-keys its macOS keychain service with a hash of
// CLAUDE_CONFIG_DIR, so a profiled launch must clear the variable and rely
// on the composed home's ~/.claude alias for configuration isolation.
func TestClaudeLaunchClearsAnInheritedConfigDirVariable(t *testing.T) {
	root := t.TempDir()
	profileRoot := filepath.Join(root, "profiles")
	projectRoot := filepath.Join(root, "project")
	binDir := filepath.Join(root, "bin")
